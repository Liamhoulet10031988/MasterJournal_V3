...
export const updateOrder = async (orderId, orderPatch) => {
  try {
    ...
    const workAmount =
      orderPatch.workAmount !== undefined ? orderPatch.workAmount : existingOrder.workAmount || 0;
    const ourPartsAmount =
      orderPatch.ourPartsAmount !== undefined
        ? orderPatch.ourPartsAmount
        : existingOrder.ourPartsAmount || 0;
    const totalAmount = workAmount + ourPartsAmount;
    ...
    const updatedOrder = {
      ...
      parts:
        orderPatch.clientParts !== undefined
          ? orderPatch.clientParts
          : existingOrder.clientParts || existingOrder.parts || '',
  ...
  try {
    ...
    orders = orders.map((order) => {
      const normalizedOrder = {
        ...
        // ИСПРАВЛЕНИЕ БАГ 1: обратная совместимость - сохраняем И amount И totalAmount
        totalAmount: order.totalAmount || order.amount || 0,
        amount: order.amount || order.totalAmount || 0, // Для старого UI
      };
