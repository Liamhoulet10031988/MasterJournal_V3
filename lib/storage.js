...
// БАГ 13: xlsx импортируется динамически только при экспорте (lazy load)
import { formatDateToStore, normalizeString } from '../utils/formatters';
...
export const saveOrder = async (orderData) => {
  try {
    ...
    const workAmount = toNumber(orderData.workAmount);
    const ourPartsAmount = toNumber(orderData.ourPartsAmount);
    const totalAmount = workAmount + ourPartsAmount;

    const newOrder = {
      id: `order_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      date: formatDateToStore(orderData.date), // ИСПРАВЛЕНИЕ: без UTC смещения
      client: normalizeString(orderData.client).trim(),
      car: normalizeString(orderData.car).trim(),
      job: normalizeString(orderData.job).trim(),
      workAmount,
      ourParts: normalizeString(orderData.ourParts).trim(),
      ourPartsAmount,
      clientParts: normalizeString(orderData.clientParts || orderData.parts).trim(),
      totalAmount,
      payType: orderData.payType,
      freonGrams: orderData.freonGrams || null,
      comment: normalizeString(orderData.comment).trim(),
...
export const updateOrder = async (orderId, orderPatch) => {
  try {
    ...
    const workAmount =
      orderPatch.workAmount !== undefined
        ? toNumber(orderPatch.workAmount, existingOrder.workAmount || 0)
        : toNumber(existingOrder.workAmount);
    const ourPartsAmount =
      orderPatch.ourPartsAmount !== undefined
        ? toNumber(orderPatch.ourPartsAmount, existingOrder.ourPartsAmount || 0)
        : toNumber(existingOrder.ourPartsAmount);
    const totalAmount = workAmount + ourPartsAmount;

    const updatedOrder = {
      ...existingOrder,
      ...orderPatch,
      date: orderPatch.date ? formatDateToStore(orderPatch.date) : existingOrder.date, // ИСПРАВЛЕНИЕ: без UTC смещения
      workAmount,
      ourPartsAmount,
      totalAmount,
      // Для совместимости
      amount: totalAmount,
      parts:
        orderPatch.clientParts !== undefined
          ? normalizeString(orderPatch.clientParts).trim()
          : normalizeString(existingOrder.clientParts || existingOrder.parts).trim(),
      updatedAt: new Date().toISOString(),
    };

    updatedOrder.client =
      orderPatch.client !== undefined
        ? normalizeString(orderPatch.client).trim()
        : normalizeString(updatedOrder.client).trim();
    updatedOrder.car =
      orderPatch.car !== undefined
        ? normalizeString(orderPatch.car).trim()
        : normalizeString(updatedOrder.car).trim();
    updatedOrder.job =
      orderPatch.job !== undefined
        ? normalizeString(orderPatch.job).trim()
        : normalizeString(updatedOrder.job).trim();
    updatedOrder.ourParts =
      orderPatch.ourParts !== undefined
        ? normalizeString(orderPatch.ourParts).trim()
        : normalizeString(updatedOrder.ourParts).trim();
    updatedOrder.clientParts =
      orderPatch.clientParts !== undefined
        ? normalizeString(orderPatch.clientParts).trim()
        : normalizeString(updatedOrder.clientParts).trim();
    updatedOrder.comment =
      orderPatch.comment !== undefined
        ? normalizeString(orderPatch.comment).trim()
        : normalizeString(updatedOrder.comment).trim();
...
export const exportCSV = async () => {
  try {
    ...
    let csv = BOM;
    csv +=
      '"Дата";"День недели";"Клиент";"Авто";"Работы (описание)";"Сумма работы";"Детали наши";"Сумма деталей наших";"Детали клиента";"Оплата";"Итого";"Фреон (г)";"Комментарий"\r\n';
...
export const exportPDF = async () => {
  try {
    ...
    try {
      ...
      throw new Error('Модуль expo-print недоступен на этой платформе');
    }

    const { printToFileAsync } = printModule || {};
    if (typeof printToFileAsync !== 'function') {
      throw new Error('expo-print недоступен в текущей сборке');
    }
    const orders = await getAllOrders();
...
export const exportXLSX = async () => {
  try {
    // БАГ 13: Динамический импорт xlsx (lazy load ~800KB)
...
export const importJSON = async (jsonString) => {
  try {
    ...
    // Генерация id, нормализация полей и обратная совместимость amount/totalAmount
    const validPayTypes = new Set(['cash', 'cashless', 'debt']);
    const importTimestamp = Date.now();

    orders = orders.map((order, index) => {
      const id = order.id || `order_${importTimestamp}_${index}`;
      const workAmount =
        order.workAmount !== undefined
          ? toNumber(order.workAmount)
          : toNumber(order.amount);
      const ourPartsAmount = toNumber(order.ourPartsAmount);
      const explicitTotal = toNumber(order.totalAmount);
      const totalAmount = explicitTotal || workAmount + ourPartsAmount;
      const payTypeCandidate = normalizeString(order.payType).trim();

      let normalizedDate = formatDateToStore(order.date || order.createdAt || new Date());
      if (!normalizedDate) {
        normalizedDate = formatDateToStore(new Date());
      }
      ...
      const normalizedOrder = {
        ...
        id,
        date: normalizedDate,
        client: normalizeString(order.client).trim(),
        car: normalizeString(order.car).trim(),
        job: normalizeString(order.job).trim(),
        workAmount,
        ourParts: normalizeString(order.ourParts).trim(),
        ourPartsAmount,
        clientParts: normalizeString(order.clientParts || order.parts).trim(),
        totalAmount,
        payType: validPayTypes.has(payTypeCandidate) ? payTypeCandidate : 'cash',
        freonGrams:
          order.freonGrams !== undefined && order.freonGrams !== null
            ? toNumber(order.freonGrams)
            : null,
        comment: normalizeString(order.comment).trim(),
    ...
    debts = debts.map((debt, index) => ({
      ...debt,
      id: debt.id || `debt_${importTimestamp}_${index}`,
      client: normalizeString(debt.client).trim(),
      car: normalizeString(debt.car).trim(),
      amount: toNumber(debt.amount),
      orderId: normalizeString(debt.orderId).trim(),
      closed: Boolean(debt.closed),
      closedAt: debt.closedAt || null,
      createdAt: debt.createdAt || new Date().toISOString(),
    }));

    // БАГ 12: Слияние заказов с обновлением существующих
    ...
    orders.forEach((order) => {
      if (existingOrdersMap.has(order.id)) {
        // Обновляем существующий заказ
        existingOrdersMap.set(order.id, { ...existingOrdersMap.get(order.id), ...order });
    ...
    // БАГ 12: Слияние долгов с обновлением существующих
    const existingDebts = await getDebts(false);
    const existingDebtsMap = new Map(existingDebts.map((d) => [d.id, d]));

    const newDebts = [];
    const updatedDebts = [];

    debts.forEach((debt) => {
      if (existingDebtsMap.has(debt.id)) {
        // Обновляем существующий долг
        existingDebtsMap.set(debt.id, { ...existingDebtsMap.get(debt.id), ...debt });
        updatedDebts.push(debt);
      } else {
        // Новый долг
        newDebts.push(debt);
        existingDebtsMap.set(debt.id, debt);
      }
    });

    const mergedDebts = Array.from(existingDebtsMap.values());
    await AsyncStorage.setItem(DEBTS_KEY, JSON.stringify(mergedDebts));

    console.log(
      `✅ Imported: ${newOrders.length} new orders, ${updatedOrders.length} updated orders, ${newDebts.length} new debts, ${updatedDebts.length} updated debts`,
    );
    return {
      importedOrders: newOrders.length,
      updatedOrders: updatedOrders.length,
      importedDebts: newDebts.length,
      updatedDebts: updatedDebts.length,
    };
  } catch (error) {
