...
export const saveOrder = async (orderData) => {
  try {
    ...
    const newOrder = {
      ...
      date: orderData.date.split('T')[0], // нормализация YYYY-MM-DD
...
export const updateOrder = async (orderId, orderPatch) => {
  try {
    ...
    const updatedOrder = {
      ...existingOrder,
      ...orderPatch,
      date: orderPatch.date ? orderPatch.date.split('T')[0] : existingOrder.date, // нормализация
      workAmount,
      ourPartsAmount,
      totalAmount,
      // Для совместимости
    ...
    // Обновляем связанный долг, если сумма изменилась и это долг
    if (updatedOrder.payType === 'debt') {
      const debt = await findDebtByOrderId(orderId);
      if (debt && !debt.closed) {
        const debts = await AsyncStorage.getItem(DEBTS_KEY);
        const debtsList = debts ? JSON.parse(debts) : [];
        const debtIndex = debtsList.findIndex(d => d.id === debt.id);
...
/**
 * Экспорт PDF с фильтрацией по датам
 * @param {Array<string>} dates - Массив дат в формате ГГГГ-ММ-ДД или null для диапазона
 * @param {string} startDate - Начальная дата диапазона (если dates === null)
 * @param {string} endDate - Конечная дата диапазона (если dates === null)
 */
export const exportPDFFiltered = async (dates = null, startDate = null, endDate = null) => {
  try {
    const { printToFileAsync } = require('expo-print');
    ...
    if (dates && Array.isArray(dates) && dates.length > 0) {
      ...
      orders = orders.filter(o => datesSet.has(o.date));
    ...
    if (orders.length === 0) {
      throw new Error('Нет заказов за выбранный период');
    }
    ...
    if (dates && dates.length > 0) {
      if (dates.length === 1) {
        periodTitle = `за ${dates[0]}`;
      } else {
        periodTitle = `за выбранные даты (${dates.length} дн.)`;
      }
    } else if (startDate && endDate) {
    ...
    <body>
      <h1>Отчёт по заказам ${periodTitle}</h1>
      <div class="date">Дата создания: ${new Date().toLocaleDateString('ru-RU')}</div>
      ...
      <div class="summary">
        <div class="summary-title">Итоговая сводка:</div>
        <table class="summary-table">
          <tr>
            <td>Всего заказов:</td>
            <td>${orders.length}</td>
          ...
          <tr style="background-color: #D9EAD3;">
            <td style="background-color: #D9EAD3;"><strong>Итого:</strong></td>
            <td style="background-color: #D9EAD3;"><strong>${totalAmount} ₽</strong></td>
          </tr>
        </table>
        
        <div class="summary-title">По типам оплаты:</div>
        <table class="summary-table">
          <tr>
            <td>Наличные:</td>
            <td>${cashTotal} ₽</td>
...
export const importJSON = async (jsonString) => {
  try {
    ...
    const ordersWithIds = imported.orders.map(order => {
      if (!order.id) {
        return {
          ...order,
          id: `order_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const newOrders = ordersWithIds.filter(o => !existingOrderIds.has(o.id));
    ...
    // Добавляем id если его нет
    const debtsWithIds = imported.debts.map(debt => {
      if (!debt.id) {
        return {
          ...debt,
          id: `debt_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    const newDebts = debtsWithIds.filter(d => !existingDebtIds.has(d.id));
